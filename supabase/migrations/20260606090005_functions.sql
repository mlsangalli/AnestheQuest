...
begin
  ...
  insert into public.analytics_aggregates (user_id, taxonomy_id, n_correct, n_total, avg_time, updated_at)
  select v_user, qt.taxonomy_id,
         case when v_correct then 1 else 0 end, 1, p_tempo_seg, now()
  ...
  on conflict (user_id, taxonomy_id) do update set
    ...
    avg_time  = case
      ...
      else (public.analytics_aggregates.avg_time * public.analytics_aggregates.n_total
            + excluded.avg_time)
           / (public.analytics_aggregates.n_total + excluded.n_total)
    end,
    updated_at = now();
